...
class UpdateBannerRequest extends FormRequest
{
    ...
    {
        return [
            'post_id'             => ['nullable', Rule::exists('posts', 'id')->where('type', PostTypeEnum::PAGE)],
            'title'               => 'string|nullable',
            'is_published'        => 'boolean',
            'type_slide'          => ['filled', Rule::in(BannerSlideTypeEnum::asArray())],
            'additional.items'               => 'filled|array',
            'additional.items.*.banner_id'   => 'sometimes|string|exists:banners,id',
            'additional.items.*.url'         => 'nullable|string',
            'additional.items.*.order'       => [
                'required',
                'integer',
            ],
